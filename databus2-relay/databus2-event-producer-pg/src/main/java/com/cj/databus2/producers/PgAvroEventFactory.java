...
public class PgAvroEventFactory {
	...
	/**
	 * PartitionFunction used to generate the event partition based on event
	 * key.
	 */
	...
	public PgAvroEventFactory(int sourceId, int pSourceId,
		...
		if (keyName == null) {
			throw new DatabusException(
					"The event schema is missing the required field \"key\".");
		}
	...
	public int createAndAppendEvent(DbChangeEntry changeEntry,
		...
		// Construct the Databus Event key, determine the key type and construct
		// the key
		...
		DbusEventInfo eventInfo = new DbusEventInfo(changeEntry.getOpCode(),
				changeEntry.getScn(), (short) _pSourceId, lPartitionId,
				changeEntry.getTimestampInNanos(), (short) _sourceId,
				schemaId.getByteArray(), payload, enableTracing, false);

		boolean success = eventBuffer.appendEvent(eventKey, eventInfo,
	...
	protected byte[] serializeEvent(GenericRecord record)
		...
		try {
			...
			_log.error("Exception for record: " + record + " with schema: "
	...
	/**
	 * Given a DBImage, returns the key If it is a single key, it returns the
	 * object if it is LONG /INT / STRING For compound key, it casts the fields
	 * as String, delimits the fields and returns the appended string
	 * 
	 * @param dbChangeEntry
