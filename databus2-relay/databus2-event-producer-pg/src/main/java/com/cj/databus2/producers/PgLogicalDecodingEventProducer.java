	...
	protected ReadEventCycleSummary readEventsFromAllSources(long sinceSCN)
		...
		try {
			pstmt = _eventSelectConnection
					.prepareStatement("SELECT * from pg_logical_slot_get_changes(?, NULL, ?, 'include-timestamp', 'on')");
		...
		Predicate<DmlEvent> predicate = new Predicate<DmlEvent>() {
			...
			public boolean apply(DmlEvent input) {
				return "public,test,schema".contains(input.getSchemaName())
						&& "user_relation, actor_relation".contains(input
								.getTableName());
			}
		};
