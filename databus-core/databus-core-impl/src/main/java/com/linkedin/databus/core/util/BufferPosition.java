 ...
 *  		   index表示单个ByteBuffer在ByteBuffers中的位置
 ...
 *		
 *	   	每当event buffer 环绕,增长全局genId.所以,让genId被编码成最标志性的位置，我们可以得到地址空间中事件的自然时间顺序。
 * <pre>
 *     ================ Important NOTE  =======================
 *
 ...
 *		
 *      ================ 切记  ================
 *		bufferPosition的值不能用来复原当event buffer重启之后。所以不要直接使用他的值在checkpointing或者其他地方，用来作为持久化relay后者client当含有event buffer的时候
 *    =========================================================
 * </pre>
 */
...
public class BufferPosition
{
   ...
   * 给offset加上增量increment，如果达到当前bytebuffer上限，则增加index，并重置offset为0
   *
   *
