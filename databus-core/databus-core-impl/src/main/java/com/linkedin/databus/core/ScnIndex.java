 ...
 *
 *  ScnIndex 也是缓冲区一种,被划分成块。每个主缓冲的区域都有一个块。每个块大小为 SIZE_OF_SCN_OFFSET_RECORD, 含有SCN 对应OFFSET
 *
 ...
 *  
 *  每个块描述了这个区域在他所在的主缓冲，区域数量公式为:num_reigions = size_of_the_scn_index_buffer/SIZE_OF_SCN_OFFSET_RECORD
 *  每个区域大小为缓冲区除以区域数
 *  
 ...
 *  
 *  为了找到SCN -- 去 SCN 缓冲寻找起始的offset(使用2进制查找) 在一个主缓冲,然后从这个offset顺序化扫描
 *
 *  Next Event will start from the next available region (this is what is returned by getLargerOffset().
 *  
 *  下一个事件将起始于下一个可用的区域(使用getLargerOffset()返回)
 *  
 *  In ScnIndex:
...
public class ScnIndex extends InternalDatabusEventsListenerAbstract
{
  ...
  //最大块数 = maxIndexSize / SIZE_OF_SCN_OFFSET_RECORD
  ...
  private void setScnOffset(int blockNumber, long scn, long offset)
  {
	  ...
	  //当多个周期在同一个块时, SCNINDex 将记录第一个周期scn在这个块。
	  ...
	  try
	  {
		  ...
		  if (lastWrittenPosition >= 0 )
		  {
			  ...
			  while ((!overwritingHead) && (currentWritePosition != blockNumber*SIZE_OF_SCN_OFFSET_RECORD))
			  {
				  ...
				  //16个字节的写,所以一个block是16字节，前八个字节是scn,后八个字节表示对应的offset
