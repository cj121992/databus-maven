...
public class BufferPositionParser
{
  ...
  //Mask对应各个位置上的掩码。例如offset 1111,index 11110000, genId 1111。。。11100000000(63-8个1)
   ...
   * individualBufferSize 是offset的位数, numIndices 是 index的位数。剩余的为genIds的位数
   *
  ...
  */
                               ...
                               boolean noLimit)
  {
    ...
    //如果当前offset >limit && okToRegress可以回退, index + 1, 使用下一个bytebuffer
    if (okToRegress)
    {
      return incrementIndex(currentPosition, buffers);
    }
  ...
  public void assertSpan(long start, long end, boolean isDebugEnabled)
  {
    ...
    //判断是否需要span(根据当前index和offset所能提供最大值)
