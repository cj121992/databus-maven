 ...
 * sent over the wire
 * 下面这个表格总结了databus events 在内存存储的格式和发送格式
 * <table border="1" >
...
implements Cloneable
{
   ...
   * <pre>
   *   Version (1 byte)               // 0 for DbusEventV1 (historical), 2 for DbusEventV2 事件版本
   *   HeaderCrc (4 bytes)            // CRC to protect the header from being corrupted 循环冗余校验
   *   Length (4 bytes)
   *   Attributes (2 bytes)           // Key-type, Trace marker, Event-opcode 自定义参数(事件类型存这儿)
   *   Sequence (8 bytes)             // Sequence number for the event window in which this event was generated (自然序列号)
   ...
   *   ValueCrc (4 bytes)             // CRC to protect the value from being corrupted 循环冗余校验
  ...
  {
	//判断bytebuffer order,大写则使用BE, 小写则使用LE
  ...
  /**
   * 序列化DbusEventInfo 2 指定格式
   * @param key
   * @param serializationBuffer
   * @param eventInfo
   * @param attributes
   * @return 占用长度
   */
  private static int serializeFullEvent(long key,
                                        ...
                                        byte[] attributes)
  {
	//获取payload和剩余长度,如果没有则获取目标存储remain
