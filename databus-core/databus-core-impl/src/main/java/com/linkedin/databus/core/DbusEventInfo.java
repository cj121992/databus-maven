...
public class DbusEventInfo
{
  ...
  /**
   * 
   * @param opCode 操作类型(update, delete)
   * @param sequenceId 序列号--->scn(可以是事务Id,必须唯一)
   * @param pPartitionId the event physical partition id
   * @param lPartitionId the event physical partition id
   * @param timeStampInNanos 创建时间戳
   * @param srcId pSourceId
   * @param payloadSchemaMd5 payload md5值
   * @param payload 序列化后的事件内容
