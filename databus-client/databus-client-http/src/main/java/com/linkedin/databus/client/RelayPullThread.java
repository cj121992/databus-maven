...
public class RelayPullThread extends BasePullThread {
	...
	private static final ArrayList<RegisterResponseEntry> EMPTY_REGISTER_LIST = new ArrayList<RegisterResponseEntry>();
	private static final ArrayList<Integer> EMPTY_STREAM_LIST = new ArrayList<Integer>();

	private final boolean _isConsumeCurrent;
	private final HttpStatisticsCollector _relayCallsStats;

	// Keep track of the last open relay connection so that we can close it on
	// shutdown
	...
	// if that much time passes with no events - reset the connection, and pick
	// another server
	private long _noEventsConnectionResetTimeSec;
	private long _timeSinceEventsSec = System.currentTimeMillis();

	private boolean _skipWTE = false;

	/**
	 * When RelayPullThread falls-off the relay, it goes to the Fell-Off mode
	 * and the below retryConfig (_retriesOnFallOff) control the retries. The
	 * fell-off mode is also set when the client starts with a bootstrap-ckpt.
	 * 
	 * <pre>
	...
	public RelayPullThread(String name, DatabusSourcesConnection sourcesConn,
			DbusEventBuffer dbusEventBuffer,
			ConnectionStateFactory connStateFactory, Set<ServerInfo> relays,
			List<DbusKeyCompositeFilterConfig> relayFilterConfigs,
			boolean isConsumeCurrent, boolean isReadLatestScnOnErrorEnabled,
			double pullerBufferUtilPct, int noEventsConnectionResetTimeSec,
			MBeanServer mbeanServer, DbusEventFactory eventFactory, Logger log,
			boolean skipWTE) {
		super(name, sourcesConn.getConnectionConfig().getPullerRetries(),
				sourcesConn, dbusEventBuffer, connStateFactory, relays,
				mbeanServer, eventFactory, log);
		_relayFilterConfigs = relayFilterConfigs;
		_isConsumeCurrent = isConsumeCurrent;
		_remoteExceptionHandler = new RemoteExceptionHandler(sourcesConn,
				dbusEventBuffer, eventFactory);
		...
		_retriesOnFallOff = new BackoffTimer("RetriesOnFallOff",
				new BackoffTimerStaticConfig(0, 0, 1, 0, sourcesConn
						.getConnectionConfig().getNumRetriesOnFallOff()));
	...
	/**
	 * Invoked when a LifeCycle message of type "SUSPEND_ON_ERROR" is received
	 * by relay puller thread as defined in
	 * AbstractActorMessageQueue#executeAndChangeState(Object)
	 * 
	 * 1. Invoke same method in super class 2. Send an "I'm dead" heartbeat
	 * value as a failsafe
	 */
	@Override
		...
		if (message instanceof ConnectionStateMessage) {
			if (_componentStatus.getStatus() != DatabusComponentStatus.Status.RUNNING) {
				...
				case CLOSED:
					...
					break;
				case PICK_SERVER:
					doPickRelay(currentState);
					break;
				case REQUEST_SOURCES:
					doRequestSources(currentState);
					break;
				case SOURCES_RESPONSE_SUCCESS:
					doSourcesResponseSuccess(currentState);
					break;
				case REQUEST_REGISTER:
					doRequestRegister(currentState);
					break;
				case REGISTER_RESPONSE_SUCCESS:
					doRegisterResponseSuccess(currentState);
					break;
				case BOOTSTRAP:
					doBootstrap(currentState);
					break;
				case REQUEST_STREAM:
					doRequestStream(currentState);
					break;
				case STREAM_REQUEST_SUCCESS:
					doReadDataEvents(currentState);
					break;
				case STREAM_RESPONSE_DONE:
					doStreamResponseDone(currentState);
					break;
				case SOURCES_REQUEST_ERROR:
					processSourcesRequestError(currentState);
					break;
				case SOURCES_RESPONSE_ERROR:
					processSourcesResponseError(currentState);
					break;
				case REGISTER_REQUEST_ERROR:
					processRegisterRequestError(currentState);
					break;
				case REGISTER_RESPONSE_ERROR:
					processRegisterResponseError(currentState);
					break;
				case STREAM_REQUEST_ERROR:
					processStreamRequestError(currentState);
					break;
				case STREAM_RESPONSE_ERROR:
					processStreamResponseError(currentState);
					break;
				default: {
					_log.error("Unknown state in RelayPullThread: "
			...
			case BOOTSTRAP_COMPLETE:
				doBootstrapComplete(bootstrapResultMessage);
				break;
	...
	private void doBootstrapFailed(BootstrapResultMessage bootstrapResultMessage) {
		_log.error("bootstrap failed",
				bootstrapResultMessage.getFailureReason());
	...
	private void doBootstrapComplete(
			BootstrapResultMessage bootstrapResultMessage) {
		Checkpoint cp = bootstrapResultMessage.getBootstrapCheckpoint();

		// we must have persisted some checkpoint at the end of the bootstrap
		// phase
		if (null == cp) {
			bootstrapResultMessage
					.switchToBootstrapFailed(new RuntimeException(
							"No persistent checkpoint found at the end of bootstrap!"));
			doBootstrapFailed(bootstrapResultMessage);
		} else {
			...
			_log.info("Bootstrap completed: " + "Consumption Mode="
					+ consumptionMode + " startScn="
					+ cp.getBootstrapStartScn() + " targetScn="
					+ cp.getBootstrapTargetScn() + " sinceScn="
					+ cp.getBootstrapSinceScn() + " windowScn="
					+ cp.getWindowScn());

			UnifiedClientStats unifiedClientStats = _sourcesConn
	...
	protected void doPickRelay(ConnectionState curState) {
		...
		if (0 == serversNum) {
			enqueueMessage(LifecycleMessage
					.createSuspendOnErroMessage(new DatabusException(
							"No relays specified")));
			return;
		}
		...
		while (null == relayConn
				&& (retriesLeft = _status.getRetriesLeft()) >= 0
				&& !checkForShutdownRequest()) {
			_log.info("picking a relay; retries left:" + retriesLeft
					+ ", Backoff Timer :" + _status.getRetriesCounter()
					+ ", Are we retrying because of SCNNotFoundException : "
					+ curState.isRelayFellOff());
			...
			for (int i = 0; i <= _curServerIdx; ++i)
				serverInfo = setIter.next();

			try {
				relayConn = _sourcesConn.getRelayConnFactory()
						.createRelayConnection(serverInfo, this,
								_remoteExceptionHandler);
				_log.info("picked a relay:" + serverInfo.toSimpleString());
			} catch (Exception e) {
				_log.error(
						"Unable to get connection to relay:"
								+ serverInfo.toSimpleString(), e);
			}
		}
		...
		if (!checkForShutdownRequest()) {
			...
			if (null == relayConn) {
				if (_currentState.isRelayFellOff()) {
					...
					try {
						enqueueMessage = onRelayFellOff(
								curState,
								curState.getCheckpoint(),
								new ScnNotFoundException(
										"Retries on SCNNotFoundException exhausted !!"));
					} catch (InterruptedException ie) {
						_log.error(
								"interrupted while processing onRelayFellOff",
								ie);
						curState.switchToPickServer();
						enqueueMessage(curState);
					} catch (InvalidEventException e) {
						_log.error(
								"error trying to notify dispatcher of bootstrapping :"
										+ e.getMessage(), e);
						curState.switchToPickServer();
						enqueueMessage(curState);
					}
					...
					try {
						...
						_remoteExceptionHandler
								.handleException(new PullerRetriesExhaustedException());
					} catch (InterruptedException ie) {
						_log.error(
								"Interrupted while processing retries exhausted",
								ie);
					} catch (InvalidEventException e) {
						_log.error(
								"Error trying to notify dispatcher of puller retries getting exhausted",
								e);
					}
					_log.error("Cannot find a relay");
				}
			} else {
				DatabusRelayConnection oldRelayConn = curState
						.getRelayConnection();
				...
				curState.switchToRequestSources(serverInfo,
						serverInfo.getAddress(), relayConn);
	...
	protected void doRequestSources(final ConnectionState curState) {
		if (null != _relayCallsStats)
			_relayCallsStats.registerSourcesCall();
	 ...
	 * */
	private String buildSubsList(List<DatabusSubscription> subs,
			Map<String, IdNamePair> sourceNameMap) {
			...
			if (!ls.idKnown() && !ls.isWildcard()) {
				...
				if (null == sourceEntry) {
					...
					throw new RuntimeException(
							"FATAL! unable to find logical source "
									+ ls.getName() + " in " + sourceNameMap);
				}
				realSub = new DatabusSubscription(sub.getPhysicalSource(),
						sub.getPhysicalPartition(), new LogicalSourceId(
								new LogicalSource(sourceEntry.getId()
										.intValue(), ls.getName()), sub
										.getLogicalPartition().getId()));
			}
	...
	protected void doSourcesResponseSuccess(ConnectionState curState) {
		...
		boolean error = false;
		List<IdNamePair> sourcesList = new ArrayList<IdNamePair>(_sourcesConn
				.getSourcesNames().size());
		if (curState.getRelayConnection().getProtocolVersion() < 3) {
			// Generate the list of sources for the sources= parameter to the
			// /stream call.
			// This is needed only for version 2 of the protocol. For version 3
			// and higher,
		...
		String subsString = error ? "ERROR" : buildSubsList(
				curState.getSubscriptions(), sourceNameMap);
		...
		_sourcesConn.getRelayDispatcher().enqueueMessage(
				SourcesMessage.createSetSourcesIdsMessage(
						curState.getSources(), sourcesIdList));
		...
		if (toTearConnAfterHandlingResponse()) {
			...
			if (error) {
				...
				curState.switchToRequestSourcesSchemas(sourcesIdList,
						subsString);
				...
				String hostHdr = curState.getHostName(), svcHdr = curState
						.getSvcName();
				_log.info("Connected to relay " + hostHdr
						+ " with a service identifier " + svcHdr);
			}
			enqueueMessage(curState);
		}
	}
	...
	protected void doRequestRegister(ConnectionState curState) {
		if (null != _relayCallsStats)
			_relayCallsStats.registerRegisterCall(EMPTY_REGISTER_LIST);
		_log.debug("Sending /sources request");
		curState.swichToRegisterRequestSent();
		sendHeartbeat(_sourcesConn.getUnifiedClientStats());
		curState.getRelayConnection().requestRegister(
				curState.getSourcesIdListString(), curState);
	}
	...
	protected void doRegisterResponseSuccess(ConnectionState curState) {
		...
		if (curState.getSourcesSchemas().size() < _sourcesConn
				.getSourcesNames().size()) {
			_log.error("Expected " + _sourcesConn.getSourcesNames().size()
					+ " schemas, got: " + curState.getSourcesSchemas().size());
			curState.switchToPickServer();
		} else {
			_sourcesConn.getRelayDispatcher().enqueueMessage(
			...
			if (toTearConnAfterHandlingResponse()) {
				...
				if (DbusClientMode.BOOTSTRAP_SNAPSHOT == cp
						.getConsumptionMode()
						|| DbusClientMode.BOOTSTRAP_CATCHUP == cp
								.getConsumptionMode()) {
					...
					if (_sourcesConn.isBootstrapEnabled()) {
						...
						String message = "bootstrap checkpoint found but bootstrapping is disabled:"
								+ cp;
						_log.error(message);
					...
					if (cp.getWindowOffset() > 0) {
						...
						_log.info("RelayPuller reconnecting when in middle of event window. Will regress. Current Checkpoint :"
								+ cp);
						if (cp.getPrevScn() > 0) {
							...
							_log.fatal("Checkpoint does not have prevSCN !!. Suspending !! Checkpoint :"
									+ cp);
							enqueueMessage(LifecycleMessage
									.createSuspendOnErroMessage(new Exception(
											"Checkpoint does not have prevSCN !!. Suspending !! Checkpoint :"
													+ cp)));
							enqueueMessage = false;
						}
					}
	...
	protected void doRequestStream(ConnectionState curState) {
		...
		int freeBufferThreshold = (int) (_sourcesConn.getConnectionConfig()
				.getFreeBufferThreshold() * 100.0 / _pullerBufferUtilizationPct);
		try {
			curState.getDataEventsBuffer()
					.waitForFreeSpace(freeBufferThreshold);
		} catch (InterruptedException ie) {
			// loop
			enqueueMessage(curState);
			return;
		...
		if (null == _relayFilter) {
			...
			if (debugEnabled)
				_log.debug("Merged Filter (before deduping) is :"
						+ _relayFilter);
			_relayFilter.dedupe();
			if (debugEnabled)
		...
		int fetchSize = (int) ((curState.getDataEventsBuffer()
				.getBufferFreeReadSpace() / 100.0) * _pullerBufferUtilizationPct);
		...
		sendHeartbeat(_sourcesConn.getUnifiedClientStats());
		curState.getRelayConnection().requestStream(args, _relayFilter,
				fetchSize, cpMult,
				_sourcesConn.getConnectionConfig().getKeyRange(), curState);
	}
	...
	protected void doReadDataEvents(ConnectionState curState) {
		...
		boolean enqueueMessage = true;
		try {
			ChunkedBodyReadableByteChannel readChannel = curState
					.getReadChannel();
			Checkpoint cp = curState.getCheckpoint();

			curState.setRelayFellOff(false);

			String remoteErrorName = RemoteExceptionHandler
					.getExceptionName(readChannel);
			Throwable knownRemoteError = _remoteExceptionHandler
					.getException(readChannel);
			...
			if (null != knownRemoteError
				...
				if (toTearConnAfterHandlingResponse()) {
					...
					if (_retriesOnFallOff.getRemainingRetriesNum() > 0) {
						_log.error("Got SCNNotFoundException. Retry ("
								+ _retriesOnFallOff.getRetriesNum()
								+ ") out of "
								+ _retriesOnFallOff.getConfig()
										.getMaxRetryNum());
						curState.switchToPickServer();
					} else {
						enqueueMessage = onRelayFellOff(curState, cp,
								knownRemoteError);
					}
				}
				...
				if (toTearConnAfterHandlingResponse()) {
					...
					_log.error("read events error: "
							+ RemoteExceptionHandler
									.getExceptionMessage(readChannel));
					curState.switchToStreamResponseError();
				}
			} else {
				/*
				 * DispatcherState dispatchState =
				 * curState.getDispatcherState();
				...
				DbusEventsStatisticsCollector connCollector = _sourcesConn
						.getInboundEventsStatsCollector();

				if (curState.isSCNRegress()) {
					_log.info("SCN Regress requested !! Sending a SCN Regress Message to dispatcher. Curr Ckpt :"
							+ curState.getCheckpoint());

					DbusEvent regressEvent = getEventFactory()
							.createSCNRegressEvent(
									new SCNRegressMessage(curState
											.getCheckpoint()));

					writeEventToRelayDispatcher(
							curState,
							regressEvent,
							"SCN Regress Event from ckpt :"
									+ curState.getCheckpoint());
				...
				UnifiedClientStats unifiedClientStats = _sourcesConn
						.getUnifiedClientStats();
				if (unifiedClientStats != null) {
					unifiedClientStats.setBootstrappingState(false); // failsafe:
																		// we're
																		// definitely
																		// not
																		// bootstrapping
				...
				int eventsNum = curState.getDataEventsBuffer().readEvents(
						readChannel, curState.getListeners(), connCollector);
				...
				if (eventsNum > 0) {
					...
					if (_remoteExceptionHandler
							.getPendingEventSize(readChannel) > curState
							.getDataEventsBuffer().getMaxReadBufferCapacity()) {
						// The relay had a pending event that we can never
						// accommodate. This is fatal error.
						String err = "ReadBuffer max capacity("
								+ curState.getDataEventsBuffer()
										.getMaxReadBufferCapacity()
								+ ") is less than event size("
								+ _remoteExceptionHandler
										.getPendingEventSize(readChannel)
								+ "). Increase databus.client.connectionDefaults.eventBuffer.maxEventSize and restart.";
						_log.fatal(err);
						enqueueMessage(LifecycleMessage
								.createSuspendOnErroMessage(new PendingEventTooLargeException(
										err)));
						return;
					} else {
						if (_noEventsConnectionResetTimeSec > 0) { // unless the
																	// feature
																	// is
																	// disabled
																	// (<=0)
							...
							if (resetConnection) {
								_timeSinceEventsSec = System
										.currentTimeMillis();
								_log.warn("about to reset connection to relay "
										+ curState.getServerInetAddress()
										+ ", because there were no events for "
				...
				// if it has been too long since we got non-empty responses -
				// the relay may be stuck, try to reconnect
	...
	protected void doStreamResponseDone(ConnectionState curState) {
		...
		if (null != _sourcesConn.getLocalRelayCallsStatsCollector()) {
			long streamDuration = System.currentTimeMillis()
					- _streamCallStartMs;
	...
	protected void doBootstrap(ConnectionState curState) {
		...
		try {
			...
			_log.error(msg, e);
			BootstrapResultMessage bootstrapResultMessage = BootstrapResultMessage
					.createBootstrapFailedMessage(e);
			doBootstrapFailed(bootstrapResultMessage);
			return;
		}

		if (!bootstrapCkpt.isBootstrapStartScnSet()) {
			bootstrapCkpt = curState.getBstCheckpointHandler()
					.createInitialBootstrapCheckpoint(bootstrapCkpt,
		...
		CheckpointMessage bootstrapCpMessage = CheckpointMessage
				.createSetCheckpointMessage(bootstrapCkpt);
		...
		try {
			...
			DbusEvent cpEvent = getEventFactory().createCheckpointEvent(
					cpForDispatcher);
			writeEventToRelayDispatcher(curState, cpEvent,
					"Control Event to start bootstrap");
			curState.switchToBootstrapRequested();
		} catch (InterruptedException ie) {
			_log.error(
					"Got interrupted while writing control message to bootstrap !!",
					ie);
		...
		/*
		 * boolean bootstrapDone = false; while (! bootstrapDone) { try {
		 * bootstrapPullerThread.join(); bootstrapDone = true; } catch
	...
	private void mergeRelayCallsStats() {
		if (null != _sourcesConn.getLocalRelayCallsStatsCollector()
			...
			_sourcesConn.getRelayCallsStatsCollector().merge(
					_sourcesConn.getLocalRelayCallsStatsCollector());
	...
	private void processSourcesRequestError(ConnectionState state) {
		...
		if (null != _relayCallsStats)
			_relayCallsStats.registerInvalidSourceRequest();
	...
	private void processRegisterRequestError(ConnectionState state) {
		...
		if (null != _relayCallsStats)
			_relayCallsStats.registerInvalidRegisterCall();
		if (toTearConnAfterHandlingResponse()) {
			tearConnectionAndEnqueuePickServer();
		} else {
			state.switchToPickServer();
	...
	private void processStreamRequestError(ConnectionState state) {
		...
		if (null != _relayCallsStats)
			_relayCallsStats.registerInvalidStreamRequest();
		if (toTearConnAfterHandlingResponse()) {
			tearConnectionAndEnqueuePickServer();
		} else {
			state.switchToPickServer();
	...
	private boolean onRelayFellOff(ConnectionState curState, Checkpoint cp,
			Throwable knownRemoteError) throws InvalidEventException,
		...
		if (!_sourcesConn.isBootstrapEnabled()) {
			...
			_log.error(" bootstrapServices="
					+ _sourcesConn.getBootstrapServices()
					+ "; bootstrapRegistrations="
					+ _sourcesConn.getBootstrapRegistrations());
			...
			if (_isReadLatestScnOnErrorEnabled) {
				...
				_remoteExceptionHandler
						.handleException(new BootstrapDatabaseTooOldException(
								knownRemoteError));
				enqueueMessage = false;
			}
		} else {
	 ...
	 * 
	 * @param noEventsConnectionResetTimeSec
	 *            - new threshold
	 */
	public void setNoEventsConnectionResetTimeSec(
			long noEventsConnectionResetTimeSec) {
		_noEventsConnectionResetTimeSec = noEventsConnectionResetTimeSec;
	}
	...
	private void writeEventToRelayDispatcher(ConnectionState curState,
			DbusEvent event, String message) throws InterruptedException,
			InvalidEventException {
		boolean success = false;

		// Create a infinite backoff timer that waits for maximum of 1 sec
		// for writing the control message to evb
		BackoffTimerStaticConfig timerConfig = new BackoffTimerStaticConfig(1,
				1000, 1, 1, -1);
		...
		while ((!success) && (timer.getRemainingRetriesNum() > 0)) {
			...
			if (ecnt <= 0) {
				_log.error("Not enough spece in the event buffer to add a control message :"
						+ message);
				boolean interrupted = !timer.backoffAndSleep();

				if (interrupted)
					throw new InterruptedException(
							"Got interrupted while waiting to write control Message to EVB : "
									+ message);
			} else {
